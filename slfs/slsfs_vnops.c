...
void
slsfs_sas_trace_update(vm_offset_t vaddr, vm_map_t map, vm_page_t m, int fault_type)
{
	if ((map->flags & MAP_SNAP_TRACE) == 0)
		return;

	slsfs_sas_attempts += 1;
...
retry:
	 ...
	 * to break loose. We control how we use the mappings so we can live
	 * with that.
	 */
...
slsfs_sas_ioctl(struct vop_ioctl_args *ap)
{
	...
	case SLSFS_SAS_MAP:
		...
		/*
		 * XXX If I understand correctly declaring the ioctl
		 * as copyout means we can update the field here
		 * and it will be propagated to userspace.
		 */
