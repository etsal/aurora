...
{
	return ((offset  - ca->ca_startoff) / CA_BLOCKS);
}
...
ca_chind_to_offset(struct chunkallocator *ca, int chind)
{
	...
	return ca->ca_startoff + (CA_BLOCKS * chind);
}
...
static void
cac_movable_add(struct ca_chunk *ch, size_t ind)
{
	mtx_assert(&ch->cac_mtx, MA_OWNED);

	KASSERT(ch->cac_movable < ch->cac_blocks_used, ("more movable blocks than used"));
	KASSERT(ch->cac_launder[ind] == NULL, ("launder page already present"));

	ch->cac_launder[ind] = hackpage;
	ch->cac_movable += 1;
	ch->cac_laundered += 1;
...
static void
cac_movable_remove(struct ca_chunk *ch, size_t ind)
{
	mtx_assert(&ch->cac_mtx, MA_OWNED);

	KASSERT(ch->cac_movable > 0, ("negative movable blocks"));
	KASSERT(ch->cac_launder[ind] != NULL, ("no launder page to remove"));
...
cac_from_cold(struct chunkallocator *ca, struct ca_chunk *ch)
{
	...
	ca_checkstate_unlocked(ch, CA_COLD);
	ch->cac_state = CA_NOQUEUE;
...
ca_init(struct chunkallocator *ca, uint64_t startoff, size_t numblocks)
{
	...
	ca_init_chunks(ca);
	for (i = 0; i < ca->ca_chunk_cnt; i++) {
		mtx_lock(&ca->ca_chunks[i].cac_mtx);
		cac_to_free(ca, &ca->ca_chunks[i]);
		mtx_unlock(&ca->ca_chunks[i].cac_mtx);
	}
...
static void
cac_print(struct ca_chunk *ch)
{
	printf("==== CHUNK %d ====\n", ch->cac_index);
	printf("Pointer: (%d, %d)\n", ch->cac_ptr.offset, ch->cac_ptr.size);
	printf("Blocks used: %ld\n", ch->cac_blocks_used);
	printf("Allocation index: %ld\n", ch->cac_alloc_index);
	printf("Clean index: %ld\n", ch->cac_clean_index);
	printf("State: %d\n", ch->cac_state);
	printf("Movable blocks: %ld\n", ch->cac_movable);
	printf("Laundered blocks: %d\n", ch->cac_laundered);
}
...
ca_print(struct chunkallocator *ca)
{
	...
	printf("Total chunks prepared for laundering: %ld\n", CA_COUNTER(ca, op_age_io));
	...
	nom = denom = 0;
	for (i = 0; i < CA_FREESLOTS; i++) {
		denom += CA_COUNTER(ca, op_alloc_fast[i]);
		nom += CA_COUNTER(ca, page_alloc_fast[i]);
	}
	...
	printf("Total data pages reclaimed: %ld\n", CA_COUNTER(ca, page_reclaim));
	printf("Total data pages queued for laundering: %ld\n", CA_COUNTER(ca, page_launder));
	...
	for (i = 0; i < CA_COLD_BUCKETS; i++) {
		TAILQ_FOREACH(ch, &ca->ca_cold[i], cac_next) {
			loadmax = ch->cac_blocks_used > loadmax ? ch->cac_blocks_used : loadmax;
			nom += ch->cac_blocks_used;
			denom += 1;
		}
	}
	printf("Cold chunks: %ld (total load %ld, average load %ld, maximum load %ld)\n", denom, nom, denom ? nom / denom : denom, loadmax);
...
static void
ca_launder_task(void *ctx, int __unused pending)
{
	struct ca_launder_task_args *args = (struct ca_launder_task_args *)ctx;
	...
	KASSERT(ch->cac_movable == 0, ("chunk has %ld movable pages", ch->cac_movable));
	...
	for (i = 0; i < CA_BLOCKS; i++) {
		...
		CA_COUNTER_INCREMENT(ca, page_reclaim);
	}
...
static void
ca_launder_chunk(struct chunkallocator *ca, struct ca_chunk *ch)
{
	struct ca_launder_task_args *args;
	...
	args->ca = ca;
	args->ch = ch;
	TASK_INIT(&args->tk, 0, ca_launder_task, &args->tk);
...
ca_gc_mktxn(struct ca_chunk *ch, size_t numblocks, struct objsnap_txn *txn, vm_page_t *ma, size_t *cnt)
{
	size_t i;
	...
	mtx_lock(&ch->cac_mtx);
	for (i = ch->cac_clean_index; i < CA_BLOCKS; i++) {
...
#endif
				cac_movable_remove(ch, i);
				printf("[%d %ld]", ch->cac_index, ch->cac_clean_index);
			}
			continue;
		}
	...
	ch->cac_clean_index = min(i, CA_BLOCKS);
...
ca_gc_move(struct chunkallocator *ca, struct ca_chunk *ch, size_t numblocks)
{
	...
	mtx_lock(&distch->cac_mtx);
	numblocks = min(numblocks, CA_BLOCKS - distch->cac_alloc_index);

	KASSERT(distch != ch, ("identical source and destination"));
	ca_gc_mktxn(ch, numblocks, &txn, ma, &cnt);
	ca_blkalloc(distch, &txn, ca);

	mtx_unlock(&distch->cac_mtx);
...
ca_gc(struct chunkallocator *ca, size_t numblocks)
{
	...
	if (ch->cac_state == CA_FREE) {
		printf("Marked as free\n");
		mtx_unlock(&ch->cac_mtx);
		mtx_unlock(&ca->ca_mtx);
		return;
	}

	ca_checkstate_unlocked(ch, CA_LAUNDER);
	ch->cac_state = CA_NOQUEUE;
	...
	if (ch->cac_state == CA_FREE || ch->cac_clean_index == CA_BLOCKS) {
		if (ch->cac_movable != 0)
...
ca_free(struct chunkallocator *ca, obj_diskptr_t ptr)
{
	...
	for (i = 0; i < ptr.size; i++) {
		...
		if (ch->cac_launder[ind] != 0) {
			cac_movable_remove(ch, ind);
#if 0
			vm_page_free(ch->cac_launder[ind]);
#endif
		}

		if (objbit[ptr.offset + i] == 0)
			printf("double free %d", ptr.offset + i);
		objbit[ptr.offset + i] = 0;
	}
	...
	if (ch->cac_blocks_used != 0) {
		if (ch->cac_state == CA_COLD && ch->cac_blocks_used < CA_COLD_LOAD_THRESHOLD) {
			cac_from_cold(ca, ch);
			ca_launder_chunk(ca, ch);
			CA_COUNTER_INCREMENT(ca, cold_to_launder);
		}
