...
static void
slsckpt_dataregion_cleanup(
    struct slsckpt_data *sckpt_data, struct slspart *slsp, uint64_t nextepoch)
{
	int stateerr;
    ...
    uint64_t *nextepoch)
{
	...
	*nextepoch = slsp_epoch_preadvance(slsp);
	...
	TASK_INIT(
	    &msnapctx->tk, 0, &slsckpt_dataregion_dumptask, &msnapctx->tk);
