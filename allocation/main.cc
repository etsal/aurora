...
void
convert_to_ba_alloc(struct binaryallocator* allocator,
...
int
writethis(std::set<uint32_t>& write_set,
          stats& st,
...
int64_t
ParetoCdfInversion(double u, double theta, double k, double sigma)
{
  double ret;
  if (k == 0.0) {
...
double
pareto_distribution(double max, double alpha)
{
        ...
        AllocType type)
{
  ...
  std::uniform_int_distribution<uint64_t> dis_nothot{ size_of_hotset,
                                                      max_obj_size };
...
// We have to imitate a random write workload, so we collect a write set and
// decide what frees to do.
stats
dowork(std::vector<diskptr_t>& allocation_map,
       std::mutex& mtx,
       void* allocator,
       AllocType type)
{
  ...
  fn << min_writes << "min-" << max_writes << "max-" << disksize / GIB << "ds-"
     << max_obj_size / GinBlocks;
  fn << "objsize-" << size_of_hotset / GinBlocks << "hot-" << maxTransactions
     << "-" << enable_old_chunks << ".csv";
  ...
  for (int i = 0; i < maxTransactions; i++) {
    if ((i != 0) && (i % print_per) == 0) {
      ...
      if (allocstats.list[allocstats.numStats - 1] !=
...
usage()
{
  ...
  printf(
    "  -t\t\tType of allocator: dt = dueling trees, chunk = chunk allocator\n");
  ...
  printf("  -e\t\tSize of the hot set in GiB (Will span 2 std deviations of "
         "the object)\n");
...
main(int argc, char* argv[])
{
  ...
  if (blocks < max_obj_size) {
    printf(
      "Disk size too small! Reduce max object size or increase disksize\n");
    return -1;
  }
