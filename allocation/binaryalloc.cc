...
addlist(struct arraylist* al, int at, diskptr_t value)
{
  ...
  memmove(&al->list[at + 1], &al->list[at], sizeof(diskptr_t) * (al->cnt - at));
  al->list[at] = value;
  al->cnt += 1;
}
...
{
  diskptr_t* newlist =
    (diskptr_t*)malloc(sizeof(diskptr_t) * to, M_ARRAY, M_WAITOK);
...
movelist(struct arraylist* dst, struct arraylist* src)
{
  ...
  dst->cnt = src->cnt;
  src->list =
    (diskptr_t*)malloc(sizeof(diskptr_t) * dst->max, M_ARRAY, M_WAITOK);
  src->cnt = 0;
}