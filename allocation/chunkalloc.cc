...
{
    printf("Chunk ptr[%d, %d], used[%d], tnx_size[%d], sectors_free[%ld], freed[%ld], max_sectors[%ld], blocks_used[%lu]\n", 
        l->ptr.offset, l->ptr.size, l->used, l->txn_size, l->sectors_free, l->freed, l->max_sectors, l->blocks_used);
    if (verbose) {
...
struct allocatorstats ca_stat(struct chunkallocator *ca)
{
    ...
    stats.numStats = 9;
    if (used_blocks !=  ((((ca->num_chunks * CHUNKSIZE) / BLOCKSIZE)) - ca->free)) {
        for (uint32_t i = 0; i < ca->num_chunks; i++) {
            struct chunk *cl = &ca->chunks[i];
            if (cl->used != FULL)
                printchunk(cl);
        ...
        printf("%lu %lu %lu %lu %lu\n", free, stats.list[1], stats.list[7], stats.list[8], used_blocks);
        assert(false);
    }
    return stats;
}
...
static int
allocate_from_chunk(struct chunkallocator *ca, chunk *chunk, struct transaction *txns, uint32_t numblocks)
{
    diskptr_t ptr;
...
// Every thread has access to their own "binary_allocator" buckets of work to do.
// This ensures every thread does an equivalent amount of work on a given allocation.
// For example if you do an allocation of 6 pages, you will do work on the 3rd bucket (2^3),
// and move old data in those buckets. If no data for that bucket type is found then
// it just grabs the oldest data on the list (index 0).
static struct chunklist*
ensure_workset(struct chunkallocator *ca, int thread_id, int numblocks, bool high_pressure = false)
{
    ...
    if (cl->chunk != NULL && (cl->chunk->sectors_free == cl->chunk->max_sectors)) {
        cl->chunk->mtx.lock();
    ...
    if (cl->chunk == NULL) {
        ...
        for (i = 0; i < ca->old_cnt; i++) {
            ...
            bool rightTxn = ca->old_chunks[i]->txn_size == (1 << bucket) || high_pressure;
...
static void
move_data(struct chunkallocator *ca, int thread_id, uint32_t numblocks, bool high_pressure = false) {
...
static int
high_pressure_alloc(struct chunkallocator *ca, struct transaction *txns, uint32_t numblocks, int flag)
{
    int error;
start_again:
    if (!flag) {
    ...
    for (int i = 0; i < ca->num_chunks; i++) {
        struct chunk *cl = &ca->chunks[i];
        cl->mtx.lock();
        if ((cl->txn_size >= numblocks) && (cl->sectors_free > 0) && cl->used != EMPTYING) {
